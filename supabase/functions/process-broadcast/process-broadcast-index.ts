...
// 4. Captures wamid from API response for delivery tracking
// 5. Updates per-recipient status + broadcast counters
//
  ...
  try {
    ...
    for (const recipient of recipients) {
      ...
      if (waToken && waPhoneId) {
        ...
        try {
          ...
          if (waRes.ok) {
            ...
            // Capture the WhatsApp Message ID for delivery tracking
      ...
      if (success) {
        ...
        await admin.from('broadcast_recipients').update(updateData)
          .eq('id', recipient.id);
      } else {
        failedCount++;
