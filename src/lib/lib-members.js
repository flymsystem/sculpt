// src/lib/members.js — Production v1
// ─────────────────────────────────────────────────────────────────
// AUDIT FIXES:
...
export async function addMember(gymId, d) {
  ...
  const saved = data || { id: 'local_' + Date.now(), ...payload, is_active: true, created_at: new Date().toISOString() };
...
export async function updateMember(memberId, gymId, u) {
  const payload = {
