...
import { escHtml, fmtDate, memberStatus, expiryDate, memberTotal, parseMemberAddons } from './helpers.js';
    ...
    <div class="settings-grid" style="align-items:start;">
      ...
      <div style="display:flex;flex-direction:column;gap:var(--space-4);">
        ...
        <div class="settings-card">
          ...
          <button class="btn btn-primary" id="btn-export-members" style="width:100%;">
      ...
      <!-- RIGHT COLUMN: Year-end + Full backup + Info -->
      <div style="display:flex;flex-direction:column;gap:var(--space-4);">
...
<div style="font-size:12px;color:#666;margin-bottom:18px;">${subtitle || 'Total records: <strong>' + rows.length + '</strong>'}</div>
${summaryBlock}
...
<div style="margin-top:30px;padding-top:14px;border-top:1px solid #ddd;font-size:10px;color:#999;text-align:center;">Generated by Flym &middot; ${escHtml(gymName)} &middot; ${escHtml(dateStr)}<br>This document contains confidential data. Handle with care.</div>
  ...
  document.getElementById('btn-export-members')?.addEventListener('click', () => {
    ...
    const headers = ['#','Name','Phone','Join Date','Plan','Expiry','Mode','Status','Type','Add-ons'];
    const rows = filtered.map((m, i) => {
      const exp = expiryDate(m);
      const addons = parseMemberAddons(m).map(a => `${a.name} (+₹${a.price})`).join(', ');
      return [i+1, m.full_name||'', m.phone||'', fmtDate(m.join_date), m.plan_name||(m.member_type==='Trial'?'Trial':'—'),
        exp?exp.toLocaleDateString('en-IN'):'—', m.payment_mode||'—', memberStatus(m), m.member_type||'Paid', addons||'—'];
    });
  ...
  document.getElementById('btn-export-payments')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!records.length) {
        ...
        const collected = filtered.filter(m=>m.payment_status==='Paid').reduce((s,m)=>s+memberTotal(m),0);
        const pending = filtered.filter(m=>m.payment_status==='Due').reduce((s,m)=>s+memberTotal(m),0);
        const headers = ['#','Member','Phone','Plan','Amount','Mode','Status','Join Date','Expiry'];
        const rows = filtered.map((m,i) => {
          const exp=expiryDate(m); return [i+1, m.full_name||'', m.phone||'', m.plan_name||'—',
            '₹'+Number(memberTotal(m)).toLocaleString('en-IN'), m.payment_mode||'—', m.payment_status||'—',
            fmtDate(m.join_date), exp?exp.toLocaleDateString('en-IN'):'—'];
        });
        ...
        const headers = ['#','Member','Phone','Plan','Amount','Mode','Date'];
        const rows = records.map((r,i) => {
          const name = r.members?.full_name || '—';
          const phone = r.members?.phone || '—';
          const paidAt = r.paid_at ? new Date(r.paid_at).toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'}) : '—';
          return [i+1, name, phone, r.plan_name||'—', '₹'+Number(parseFloat(r.amount)||0).toLocaleString('en-IN'), r.payment_mode||'—', paidAt];
        });
  ...
  document.getElementById('btn-export-yearend')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const yearExpenses = await getExpensesByRange(S.gym.id, startDate, endDate);

      // Revenue by month
      const monthlyData = [];
      for (let mo = 0; mo < 12; mo++) {
        const mStart = new Date(year, mo, 1);
        const mEnd = new Date(year, mo + 1, 0);
        const label = mStart.toLocaleDateString('en-IN', { month: 'short' });
        const rev = S.members.filter(m => m.payment_status === 'Paid' && (m.member_type||'Paid') !== 'Trial' && m.join_date && (() => {
          const jd = new Date(m.join_date + 'T00:00:00'); return jd >= mStart && jd <= mEnd;
        })()).reduce((s,m) => s + (parseFloat(m.plan_price)||0), 0);
  ...
  document.getElementById('btn-full-backup')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const backup = {
        ...
        members: S.members.map(m => ({ full_name:m.full_name, phone:m.phone, email:m.email, date_of_birth:m.date_of_birth, gender:m.gender, plan_name:m.plan_name, plan_price:m.plan_price, member_type:m.member_type, payment_mode:m.payment_mode, payment_status:m.payment_status, join_date:m.join_date, expiry_date:m.expiry_date, member_addons:m.member_addons, notes:m.notes, is_active:m.is_active })),
  ...
  document.getElementById('btn-full-backup-pdf')?.addEventListener('click', async () => {
    ...
    try {
      ...
      // Members section
      const mHeaders = ['Name','Phone','Plan','Price','Type','Payment','Status','Join','Expiry'];
      const mRows = S.members.map(m => [
        m.full_name||'', m.phone||'', m.plan_name||'—',
      ...
      // Payment history
      const phHeaders = ['Date','Member','Plan','Amount','Mode'];
      ...
      const totalRev = S.members.filter(m=>m.payment_status==='Paid'&&(m.member_type||'Paid')!=='Trial').reduce((s,m)=>s+(parseFloat(m.plan_price)||0),0);
...
<div style="display:flex;gap:24px;margin-bottom:18px;padding:10px 14px;background:#f4f7fb;border-left:3px solid #1A6FD4;border-radius:4px;">
