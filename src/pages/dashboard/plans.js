...
import { escHtml, escAttr, parsePlanData } from './helpers.js';
  ...
  return `<div class="plan-card">
    ...
    <div class="plan-card-footer">
      ...
      <div class="action-btns">
        <button class="btn btn-ghost btn-sm" onclick="window._editPlan('${escAttr(p.id)}')">Edit</button>
        <button class="btn btn-danger-soft btn-sm" onclick="window._delPlan('${escAttr(p.id)}')">Delete</button>
      </div>
    </div>
  </div>`;
}
...
function renderPlansShowcase(c) {
  ...
  c.innerHTML = `<div class="content-inner page-enter">
    <div style="text-align:center;padding:32px 20px 24px;">
      ${S.gym?.logo_url ? `<img src="${escHtml(S.gym.logo_url)}" alt="" style="width:56px;height:56px;border-radius:12px;object-fit:cover;margin-bottom:12px;">` : ''}
